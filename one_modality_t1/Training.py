...
def main(args):
    ...
    for i in range(len(t1)):
        train_files = train_files + [{"image": t,"label": seg} for t, seg in zip(t1[i:i+1], segs[i:i+1])]
    for j in range(len(t1_val)):
        val_files = val_files + [{"image": t,"label": seg} for t, seg in zip(t1_val[j:j+1], segs_val[j:j+1])]

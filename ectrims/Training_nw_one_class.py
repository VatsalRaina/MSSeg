...
def main(args):
    ...
    for epoch in range(epoch_num):
        ...
        lr , epoch_loss=train_one_epoch_one_class(model, train_loader, device, optimizer, scheduler, loss_function, epoch, act, val_loader, thresh)
        lrs.append(lr)
        epoch_loss_values.append(epoch_loss)
        print(f"epoch {epoch + 1} average train loss: {epoch_loss:.4f}")
        
        # early stopping
        current_loss = validation_one_class(model, act, val_loader, loss_function, device, thresh, only_loss=True)
        ...
        if (epoch + 1) % val_interval == 0:
            val_loss, val_dice = validation_one_class(model, act, val_loader, loss_function, device, thresh, only_loss=False)
            metric_values.append(val_dice)
            val_loss_values.append(val_loss)
            
            train_loss, train_dice = validation_one_class(model, act, val_train_loader, loss_function, device, thresh, only_loss=False)

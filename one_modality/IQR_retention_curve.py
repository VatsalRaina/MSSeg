...
from utils.metrics import get_metric_for_rf
...
parser.add_argument('--perf_metric', type=str, default="dsc_norm",
                    help='name of a single performance metric')
...
def main(args):
    ...
    with torch.no_grad():
        for count, batch_data in enumerate(val_loader):
            ...
            # Get real, ideal, random dice score for different retention fractions
            metric_rf = get_metric_for_rf(gts=gt.flatten(),
                                          preds=seg.flatten(),

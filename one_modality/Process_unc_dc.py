...
def get_unc_score_dice_norm(gts, preds, uncs):
    ordering = uncs.argsort()
    uncs = uncs[ordering]
    gts = gts[ordering]
    preds = preds[ordering]
    ...
    for frac in fracs_retained:
        ...
        if pos == N:
            ...
            curr_preds = np.concatenate((preds[:pos], gts[pos:]))
        dsc_norm, _, _ = dice_norm_metric(gts, curr_preds)
        dsc_norm_scores.append(dsc_norm)
    dsc_norm_scores = np.asarray(dsc_norm_scores)
...
#    print(test_files)
    ...
    if K > 1:
        for i, model in enumerate(models):
            pthfilepath = os.path.join(root_dir, "seed" + str(i+1), "Best_model_finetuning.pth")
            model.load_state_dict(pthfilepath)
            model.eval()
    elif K == 1:
    ...
    with torch.no_grad():
        for count, batch_data in enumerate(val_loader):
            ...
            uncs = ensemble_uncertainties_classification(
                    np.concatenate(
                            (np.expand_dims(all_outputs, axis=-1), np.expand_dims(1.-all_outputs, axis=-1)),
                            axis=-1)
                    )
            ...
            for unc_key, curr_uncs in uncs.items():
                if unc_key == "reverse_mutual_information":
                    auc_dsc_norm = get_unc_score_dice_norm(gt.flatten(), seg.flatten(), curr_uncs.flatten())
